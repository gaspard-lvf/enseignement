...
\begin{multicols}{2}
\section{Coefficient directeur et ordonnée à l'origine}
\begin{exo}[][]
    ...
    \begin{enumerate}
        ...
        \item Écrire une formule qui donne le prix de facturation du plombier en fonction de la durée d'intervention.
    \end{enumerate}
\end{exo}

\begin{exo}[][]
    Pour chacuns des cas ci-dessous, on cherche à modéliser les situations suivantes par des fonctions affines.
    \begin{enumerate}
        \item Un artisan vend du tissus. Il facture ses clients comme indiqué ci-dessous.
                ...
                \begin{axis}[
                    ...
                    xtick={1,...,3},
                    minor y tick num=1,
                    xlabel = \textbf{Longueur de tissus vendue (en mètres)},
                    ylabel = \textbf{Prix payé (\euro)},
                    xmin = 0,
                    xmax = 3,
                    ymin = 0,
            ...
            \end{tikzpicture}
            \begin{enumerate}
                \item Quels sont les frais fixes de la vente ?
                \item Quel est le prix du tissu au mètre ?
                \item Modéliser le prix du tissu en fonction du nombre de mètres vendus par une fonction affine \(P\).
            \end{enumerate}
        \item Un réservoir contient de l'eau. Une pompe retire de l'eau à débit constant. La quantité d'eau restante est représentée sur le graphique ci-dessous.
        \begin{tikzpicture}[scale=0.8]
                \begin{axis}[
                    height=6cm,
                    width=0.5\textwidth,
                    ...
                    ymajorgrids=true,
                    minor y tick num=1,
                    xlabel = \textbf{Durée de fonctionnement (en heures)},
                    ylabel = \textbf{Quantité d'eau restante (litres)},
                    xmin = 0,
                    xmax = 5,
                    ymin = 0,
                    ymax = 550
            ...
            \end{tikzpicture}
            \begin{enumerate}
                \item Quelle était la quantité d'eau initiale dans le réservoir ?
                \item Quel est le débit de la pompe (c'est-à-dire le nombre de litres retirés par heure) ?
                \item Modéliser la quantité d'eau restante en fonction du nombre d'heures par une fonction affine \(Q\).
                \item Quand le réservoir sera-t-il vide ?
            \end{enumerate}
        \item En 2006, la population d'éléphants d'Afrique était de 526 milliers. Chaque année, elle baisse de \(1,1\) milliers.\begin{enumerate}
            \item Écrire une fonction affine \(P\) qui donne le nombre d'éléphants en fonction de l'année depuis 2006.
            \item Quelle sera la population en 2016 ?
            \item D'après ce modèle, quand la population d'éléphants sera-t-elle éteinte ?
        \end{enumerate}
    \end{enumerate}
\end{exo}
...
\begin{exo}
On considère la fonction affine \(k\) définie sur \(R\) par \(k(x) = -2x + 1\) où \(x\) est un nombre.
\begin{enumerate}
    ...
    \begin{center}
        ...
        \begin{tabular}{l|c|c|c|c|c|c}
           \(x\)    & -1 & \(-\frac{1}{2}\) & 0 & \(\frac{1}{2}\) & 1 & 2 \\ \hline
           \(k(x)\) & & & & & & \\
        \end{tabular}
    \end{center}
...
\begin{exo}
    On a représenté ci-dessous quatre fonctions affines : 
    \begin{tasks}(2)
...
\begin{enumerate}
    \item Dresser le tableau de signe de ces quatre fonctions
    \item À partir du tableau de signe d'une fonction affine, peut-on déduire son sens de variation ?
\end{enumerate}
\end{exo}
...
\begin{exo}[][]
    Proposer une fonction affine \(g\) dont le tableau de signe est le suivant :
        \begin{tikzpicture}
        \tkzTabInit[espcl=1]{$x$ / 1 ,  \(g(x) \) / 1}{$-\infty $, \(2\), \(+\infty \)}
        \tkzTabLine{,-, z, +}
    \end{tikzpicture}
\end{exo}
...
\begin{exo}[Méthode][]
    ...
    On considère la fonction \(k\) définie sur \(\R\) par \(k(x) = -5(x-4)(x+2)\)
    \begin{enumerate}
        \item Faire les tableaux de signe sur \(\R\) des fonctions \(f(x) = x-4\) et \(g(x) =x+2\).
        \item Compléter le tableau suivant pour avoir le signe de la fonction \(k\)
    \end{enumerate}

    \begin{tikzpicture}
        \tkzTabInit[espcl=2]{$x$ / 1 , -5 / 1 , \(x-4\) / 1, \(x+2\) / 1, \(k(x)\) / 1}{$-\infty $, $\dots$, \(\dots\), \(+\infty \)}
        \tkzTabLine{, , , , }
        \tkzTabLine{, , , , }
        \tkzTabVar{, , , , }
    \end{tikzpicture}
...
\begin{exo}[][appro]
    ...
    \begin{tikzpicture}
        \tkzTabInit[espcl=1]{$x$ / 1 ,  \( \) / 1}{$-\infty $, \(-3\), \(5\), \(+\infty \)}
        \tkzTabLine{,+, z, -, z, +}
    \end{tikzpicture}

    Trouver deux expressions algébriques de fonctions qui ont ce tableau de signes.
\end{exo}

\begin{exo}[Vrai-faux][avance]

    \begin{itemize}
        \item \textbf{Affirmation 1 : } On considère la fonction \(g\) définie par \(g(x) =(x+1)(x+5)\) \\
        Son tableau de signe est le suivant :

            \begin{tikzpicture}
                \tkzTabInit[espcl=1]{$x$ / 1 ,  \(g(x)\) / 1}{$-\infty $, \(1\), \(5\), \(+\infty \)}
                \tkzTabLine{,+, z, -, z, +,}
            \end{tikzpicture}
        \item \textbf{Affirmation 2 : } On considère la fonction \(h\) définie par \(h(x) =-5(x+1)(x-12)\) \\
        Son tableau de signe est le suivant :

            \begin{tikzpicture}
                \tkzTabInit[espcl=1]{$x$ / 1 ,  \(h(x)\) / 1}{$-\infty $, \(-1\), \(12\), \(+\infty \)}
                \tkzTabLine{,-, z, +, z, -}
            \end{tikzpicture}
        \item \textbf{Affirmation 3 : } On considère la fonction \(k\) définie par \(k(x) =x^2-3x+2\) \\
        Son tableau de signe est le suivant :

            \begin{tikzpicture}
                \tkzTabInit[espcl=1]{$x$ / 1 ,  \(k(x)\) / 1}{$-\infty $, \(1\), \(2\), \(+\infty \)}
                \tkzTabLine{,+, z, -, z, +}
            \end{tikzpicture}
    \end{itemize}
