...
\begin{enumerate}[leftmargin=*]
    ...
    \item Si on a définie une relation entre \(u_{n+1}\) et \(u_n\) on a défini la suite \(u_n\).
    \item Si une suite est définie par récurrence, alors il est nécessaire de calculer \(u_0, u_1, \ldots, u_9\) pour connaître \(u_{10}\).
    \item Si on connait les deux relations suivantes \(u_1 = 0\) et \(u_{n+1} = u_n +4\). On a \(u_4=16\)
\end{enumerate}
\end{exo}
