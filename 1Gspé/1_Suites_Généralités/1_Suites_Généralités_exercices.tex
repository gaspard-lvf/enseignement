...
\begin{exo}
    Pour chacune des suites suivantes calculer $u_1$, $u_2$ et $u_3$ et préciser leur mode de définition. Justifier vos réponses par une phrase.

    \begin{enumerate}
        \item $u_n=\frac{n}{2}+\frac{3}{4}$
    
        \item $u_0 = 2$ et $u_{n+1}  =  \frac{u_n}{2}+\frac{3}{4}$
...
Les affirmations suivantes portent sur les modes de définition explicite et par récurrence d'une suite. Dire si chacune est vraie ou fausse, en justifiant.

\begin{enumerate}[leftmargin=*]
    \item Si une suite \((u_n)\) est définie explicitement, on peut calculer \(u_{50}\) sans connaître \(u_{49}\).
    \item La suite définie par \(u_n = 7n-3\) est définie par récurrence.
    \item Pour définir une suite par récurrence, il suffit de donner la relation entre \(u_{n+1}\) et \(u_n\), sans préciser \(u_0\).
    \item Si une suite est définie par récurrence, il est nécessaire de calculer \(u_0, u_1, \ldots, u_9\) pour connaître \(u_{10}\).
    \item On considère la suite définie par \(v_3 = 0\) et \(v_{n+1} = v_n +4\). On a \(v_4=16\)
\end{enumerate}
\end{exo}
...
\begin{enumerate}[leftmargin=*]
    \item Proposer une expression explicite \(u_n = f(n)\), la plus simple possible, cohérente avec ces quatre valeurs.
...
\prof{
\begin{itemize}
    \item Question 1 : réponse attendue \(u_n = 2n+1\) (ou toute expression équivalente).
    \item Question 2 : réponse attendue \(u_0=1\) et \(u_{n+1}=u_n+2\).
    \item Question 3 : le facteur \(n(n-1)(n-2)(n-3)\) s'annule pour \(n=0,1,2,3\), donc l'expression proposée coïncide bien avec \(2n+1\) sur ces quatre valeurs.
    \item Question 4 : les deux expressions sont différentes : dès \(n=4\), on obtient \(2\times4+1=9\) contre \(9 + 4\times3\times2\times1 = 33\). Deux formules distinctes peuvent donc donner exactement les mêmes premiers termes.
