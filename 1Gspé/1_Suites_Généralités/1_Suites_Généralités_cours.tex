...
\begin{defi}
    Une \textbf{suite numérique} est une fonction \(u\) définie sur \(\N\) (ou sur une partie de \(\N\) de la forme \(\{n\in\N \mid n\geq n_0\}\)), à valeurs réelles. L'image d'un entier naturel \(n\) par cette fonction est notée \(u_n\) plutôt que \(u(n)\) : c'est le \textbf{terme de rang \(n\)} (ou terme d'indice \(n\)) de la suite. On note la suite \((u_n)\), ou \((u_n)_{n\in\N}\).
\end{defi}
