...
\prof{
    ...
    Pour cela , on motive l'introduction de ces notions par la volonté de comparer des effectifs de populations différentes. Exemple nombre de panier réussi sur le nombre de panier tenté pour un joueur de basket. On aimerait savoir pendant quel entraînement le joueur a été le plus adroit.
    On utilise les tableaux de proportionnalité pour mettre les proportions sous forme de pourcentage.
...
\prof{
    \begin{enumerate}
        \item Varier les manières de répondre aux questions
        \item "intuition" : "50 c'est la moitié de 100, donc 50\% de 300 c'est la moitié de 300, soit 150".
        \item Tableau de proportionnalité.
        \item 50\% de 300 = \(\frac{50}{100}\times 300\). Le de se réécrit \(\times\).
        \item 
    \end{enumerate}
}
...
\end{exo}
\prof{
    Pour l'instant utiliser un tableau de proportionnalité pour retrouver un effectif total à partir d'un sous-effectif.
}
...
\vspace{-0.5cm}\begin{exo}[][]
    Sarah, joueuse de basket professionnelle, s'entraîne tous les jours au lancer franc. Quel jour a-t-elle été le plus habile ? Et le moins habile ?
...
\begin{exo}
    ...
    \begin{enumerate}
        \item Quelle est la proportion de lycées publics parmi tous les lycées de l'académie ?
